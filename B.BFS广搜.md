# 🚀 BFS、优先队列与拓扑排序 核心笔记

## 一、 标准 BFS（适用于等权图 / 迷宫最短路）

### 1. 核心流程（五步走）
1. **初始化**：标记起点为已访问，将起点入队。
2. **循环遍历**：当队列非空时，出队队首节点，访问该节点。
3. **重点防坑**：一定要判断之前走过的地方不能再走（使用 `vis` 或 `path` 数组）。
4. **扩展节点**：遍历队首节点的所有合法邻接节点，**若未访问则标记为已访问并入队**。
5. **终止条件**：到达终点返回步数，或队列为空（所有可达节点遍历完毕）返回无解。

### 2. `std::queue` 常用 API
```cpp
queue<int> q; // 定义队列，存储节点编号或结构体

q.push(val);  // 队尾入队：将 val 加入队列尾部（扩展邻接节点时用）
q.pop();      // 队首出队：删除队列第一个元素（无返回值！处理完当前节点后弹出）
q.front();    // 获取队首：返回队首元素的引用（获取当前要处理的节点）
q.empty();    // 判空：返回 bool（BFS 循环条件 while(!q.empty()) 用）
q.size();     // 队列大小：返回队列中元素的个数
```
> ⚠️ **注意**：`q.pop()` 无返回值，仅用于删除队首元素。如果队列中需要存储多个维度的信息（如坐标+步数），请使用 `struct`。

### 3. 迷宫最短路标准模板
```cpp
#include <iostream>
#include <queue>
using namespace std;

struct locate {
    int x, y, step;
    locate(int _x, int _y, int _s) : x(_x), y(_y), step(_s) {}
};

int dx[4] = {0, 1, 0, -1};
int dy[4] = {1, 0, -1, 0};
int num[1005][1005], path[1005][1005]; 

bool is_valid(int tx, int ty, int n, int m) {
    return tx >= 0 && tx < n && ty >= 0 && ty < m && 
           num[tx][ty] == 1 && path[tx][ty] == 0;
}

int BFS(int start_x, int start_y, int n, int m) {
    queue<locate> q; 
    path[start_x][start_y] = 1; 
    q.push(locate(start_x, start_y, 0));
    
    while (!q.empty()) {
        locate now = q.front(); 
        q.pop();                
        
        if (now.x == n - 1 && now.y == m - 1) return now.step;
        
        for (int i = 0; i < 4; i++) { 
            int tx = now.x + dx[i];
            int ty = now.y + dy[i];
            if (is_valid(tx, ty, n, m)) {
                path[tx][ty] = 1; // ⚠️ 关键：加入队列前立刻标记为已走过
                q.push(locate(tx, ty, now.step + 1));
            }
        }
    }
    return -1; 
}
```

---

## 二、 优先队列（适用于带权图 / Dijkstra 雏形）

### 1. 为什么带权图不能用普通 BFS？
- **无权图**：每一步花费相同。普通 BFS 按“步数优先”，首次到达终点的必定是最短路径。
- **带权图**：每一步花费不同。步数少的路径总花费可能更大。必须使用优先队列，每次弹出「**当前总花费最小**」的节点进行扩展。

### 2. 带权最短路结构体模板 (以 P3956 棋盘为例)
```cpp
#include <queue>
using namespace std;

struct node {
    int x, y;  // 核心1：当前所在的网格坐标
    int c;     // 核心2：当前坐标的颜色
    int w;     // 核心3：到达当前坐标的总花费（排序的核心依据）

    // ⚠️ 核心技巧：重载小于号，实现小顶堆
    // 记忆口诀：想要小顶堆，就用大于号 (>)
    bool operator<(const node& b) const { 
        return w > b.w; 
    }
};

priority_queue<node> q; // 直接这样写就是小顶堆了！
```

---

## 三、 拓扑排序（适用于有向无环图 DAG 的状态转移 / 求和）

### 1. 为什么不能用普通 BFS 盲目遍历？
在有向无环图中（如排水系统），一个节点可能有多个上游。如果用普通 BFS（碰到就入队），下游节点会被重复计算成千上万次，导致 **指数级爆炸 (TLE)**。
**核心思想**：引入 `in_degree`（入度）。必须等一个节点的所有前置条件都满足了（即**入度减为 0**），才能将它入队并往下游传递数据。保证每个节点**只进队一次，只出队一次**！

### 2. 拓扑排序考场三步曲（肌肉记忆模板）
```cpp
#include <iostream>
#include <vector>
#include <queue>
using namespace std;

vector<int> road[100005];
int in_degree[100005]; // 灵魂数组：记录每个点的入度

void topo_sort(int n) {
    // 第一步：建图时，顺手统计入度（在 main 函数的输入阶段完成）
    // cin >> u >> v;
    // road[u].push_back(v);
    // in_degree[v]++;  // 核心：只要有人指向 v，v 的入度就加 1

    // 第二步：开局，把所有“天生入度为 0”的点塞进队列
    queue<int> q;
    for (int i = 1; i <= n; i++) {
        if (in_degree[i] == 0) {
            q.push(i); // 比如排水系统的“污水接收口”
        }
    }

    // 第三步：BFS 循环（剥洋葱）
    while (!q.empty()) {
        int u = q.front();
        q.pop();
        
        // 此时 u 的所有前置条件已满足，可以在这里处理 u 的最终数据！
        
        for (int v : road[u]) {
            // 1. 把 u 的数据传递给 v (状态转移)
            // water[v] += water[u] / 出边数; 
            
            // 2. 核心：v 的依赖减少了一个
            in_degree[v]--; 
            
            // 3. 核心：如果 v 的依赖全没了，入队！
            if (in_degree[v] == 0) {
                q.push(v);
            }
        }
    }
}
```
